...
const DEFAULT_INITIAL_JOURNEYS = [
  ...
  },
  {
    ...
    budgetINR: 55000,
    progress: 40,
    img: WIKIMEDIA_REAL_IMAGES['udaipur'],
...
export default function Trips() {
  ...
  // ── Proceed to Bookings (Flights & Stay) ──
  ...
  const handleConfirmPayment = () => {
    ...
    const mainCity = selectedItineraryTrip.cities?.[0]?.name || selectedItineraryTrip.dest
    const hotelCost = liveBudgetCalculation?.hotelCost || 30000
    const transportCost = liveBudgetCalculation?.transportCost || 15000
            ...
            <div className="itin-back-nav">
              ...
              <div className="itin-top-action-pills">
                ...
                <button
                  ...
                  style={{ background: 'linear-gradient(135deg, #D4A843 0%, #B38628 100%)', color: '#FFFFFF', fontWeight: 600 }}
                >
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                    <PlaneIcon size={14} color="#FFFFFF" /> Book Flights & Stay →
                  </span>
                </button>
              ...
              <div className="itin-glass-card">
                ...
                <div className="glass-actions-row">
                  <button
                    ...
                    onClick={handleProceedToBooking}
                    style={{ background: '#D4A843', color: '#1A1A1A', fontWeight: 700 }}
                  >
                    <span>Book Flights & Stay Now</span>
                    <span>→</span>
                  </button>
                  ...
                  {liveBudgetCalculation && (
                    <div className="itin-budget-widget">
                      ...
                      <button
                        ...
                        onClick={handleProceedToBooking}
                        style={{ background: 'linear-gradient(135deg, #D4A843 0%, #B38628 100%)', color: '#FFFFFF', fontWeight: 700, marginBottom: 8 }}
                      >
                        Book Flights & Stay for this Trip →
                      </button>
              ...
              <form onSubmit={handleRunAIOptimizer} className="cm-form-grid">
                ...
                <div className="cm-actions">
                  ...
                  <button type="submit" className="cm-btn-primary" disabled={isOptimizing}>
                    {isOptimizing ? '⚡ Optimizing Itinerary with PlanYatri AI...' : '⚡ Generate Optimized Itinerary →'}
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="trips-modal-card" style={{ maxWidth: '540px', padding: '28px', border: '1px solid rgba(212,168,67,0.35)', background: '#0C1B2A', color: '#FBF9F5', borderRadius: '16px' }} onClick={(e) => e.stopPropagation()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '16px' }}>
                <div>
                  ...
                  <h3 style={{ fontSize: '20px', fontWeight: 600, color: '#FFF', margin: '4px 0 0' }}>{selectedItineraryTrip.dest}</h3>
                </div>
                <button style={{ background: 'none', border: 'none', color: '#A0AEC0', fontSize: '20px', cursor: 'pointer' }} onClick={() => !isProcessingPayment && setShowPaymentModal(false)}>✕</button>
              </div>

              {/* Price Breakdown */}
              <div style={{ background: 'rgba(255,255,255,0.04)', padding: '16px', borderRadius: '12px', marginBottom: '20px', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '13px', color: '#CBD5E1' }}>
                  <span>5-Star Luxury Stay & Villa</span>
                  <span>₹{(liveBudgetCalculation?.hotelCost || 30000).toLocaleString('en-IN')}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '13px', color: '#CBD5E1' }}>
                  <span>Inter-City Express Flights</span>
                  <span>₹{(liveBudgetCalculation?.transportCost || 15000).toLocaleString('en-IN')}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', fontSize: '13px', color: '#CBD5E1' }}>
                  <span>Concierge & Safety Escort Insurance</span>
                  <span style={{ color: '#10B981', fontWeight: 600 }}>INCLUDED FREE</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.1)', fontSize: '16px', fontWeight: 700, color: '#FFF' }}>
                  <span>Total Amount Payable</span>
                  <span style={{ color: '#D4A843' }}>₹{((liveBudgetCalculation?.hotelCost || 30000) + (liveBudgetCalculation?.transportCost || 15000)).toLocaleString('en-IN')}</span>
                </div>
              </div>
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      borderRadius: '8px',
                      border: paymentMethod === m.id ? '1px solid #D4A843' : '1px solid rgba(255,255,255,0.1)',
                      background: paymentMethod === m.id ? 'rgba(212,168,67,0.15)' : 'transparent',
                      color: paymentMethod === m.id ? '#D4A843' : '#94A3B8',
                      cursor: 'pointer',
                    }}
                  >
                    {m.label}
              ...
              {paymentMethod === 'upi' && (
                <div style={{ textTransform: 'center', textAlign: 'center', background: '#FFF', padding: '16px', borderRadius: '12px', marginBottom: '20px' }}>
                  <p style={{ fontSize: '12px', color: '#64748B', margin: '0 0 8px', fontWeight: 600 }}>Scan QR with GPay / PhonePe / Paytm</p>
                  <svg width="100" height="100" viewBox="0 0 100 100" fill="none">
                    <rect width="100" height="100" fill="white"/>
                    <path d="M10 10h30v30H10zM60 10h30v30H60zM10 60h30v30H10z" fill="#0C1B2A"/>
                    <path d="M20 20h10v10H20zM70 20h10v10H70zM20 70h10v10H20z" fill="white"/>
                    <path d="M50 10h5v80h-5zM10 50h80v5h-80zM60 60h15v15H60zM75 75h15v15H75z" fill="#D4A843"/>
                  </svg>
                  <p style={{ fontSize: '11px', color: '#94A3B8', margin: '6px 0 0' }}>UPI ID: planyatri@razorpay</p>
                </div>
              )}

              {paymentMethod === 'card' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                  <input type="text" placeholder="Card Number (4000 1234 5678 9010)" style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#FFF', fontSize: '13px' }} defaultValue="4532 •••• •••• 8821" />
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <input type="text" placeholder="MM/YY" style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#FFF', fontSize: '13px' }} defaultValue="12/28" />
                    <input type="password" placeholder="CVV" style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: '#FFF', fontSize: '13px' }} defaultValue="882" />
                  </div>
                </div>
              )}
              ...
              >
                {isProcessingPayment ? '🔒 Authorizing Luxury Checkout...' : `Pay & Confirm Reservations (₹${((liveBudgetCalculation?.hotelCost || 30000) + (liveBudgetCalculation?.transportCost || 15000)).toLocaleString('en-IN')}) →`}
              </button>
            </div>
          </div>
        )}
