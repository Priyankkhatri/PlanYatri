...
export default function Topbar({ title, subtitle }: { title?: string; subtitle?: string }) {
  ...
  const userAvatar = userInfo?.avatar || 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=80&q=80&fit=crop&crop=face';
  ...
  const markAllRead = () => setNotifs((prev: any) => prev.map((n: any) => ({ ...n, unread: false })));

  const renderNotifIcon = (type: any) => {
    switch (type) {
  ...
  const renderSearchIcon = (type: any) => {
    switch (type) {
      ...
      <div className="tb-right">
        ...
        <div className="tb-search-wrap" ref={searchRef}>
          ...
          {searchOpen && (
            <div className="tb-search-dropdown">
              ...
              {SEARCH_SUGGESTIONS
                .filter((s: any) => !searchVal || s.label.toLowerCase().includes(searchVal.toLowerCase()))
                .map((item: any, i: any) => (
                  <div key={i} className="tb-search-item" onClick={() => { setSearchVal(item.label); setSearchOpen(false); router.push('/destinations'); }}>
          ...
          {notifOpen && (
            <div className="tb-notif-dropdown">
              ...
              <div className="tb-notif-list">
                {notifs.map((n: any) => (
                  <div key={n.id} className={`tb-notif-item ${n.unread ? 'tb-notif-item--unread' : ''}`} onClick={() => { setNotifs((prev: any) => prev.map((x: any) => x.id === n.id ? { ...x, unread: false } : x)); }}>
        ...
        <div className="tb-user-wrap">
          <button className="tb-user-chip" onClick={() => { setUserOpen(p => !p); setNotifOpen(false); }}>
            <img src={userAvatar} className="tb-avatar" alt={userName} />
            <div className="tb-user-info">
              <p className="tb-user-name">Hi, {userName}</p>
          ...
          {userOpen && (
            <div className="tb-user-menu">
              {[
                ...
                <button key={item.label} className="tb-menu-item" onClick={() => { router.push(item.path); setUserOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {item.icon} {item.label}
                </button>
              ))}
              <div className="tb-menu-div" />
              <button className="tb-menu-item tb-menu-item--danger" onClick={() => router.push('/')} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
