...
export default function Sidebar({ activeNav, setActiveNav }: { activeNav?: any; setActiveNav?: any } = {}) {
  ...
  const { dark, toggle }: any = useTheme();
  ...
  return (
    <motion.aside
      ...
      transition={{ type: 'spring', stiffness: 300, damping: 28, delay: 0.05 } as any}
    >
      ...
      <nav className="sb-nav">
        {NAV.map((item: any) => {
          const isActive = pathname === item.path || activeNav === item.id;
          return (
            <motion.button
              key={item.id}
              ...
              whileTap={{ scale: 0.96 }}
              transition={{ type: 'spring', stiffness: 400, damping: 22 } as any}
            >
      ...
      {!collapsed && (
        <div className="sb-bottom">
          ...
          <div className="sb-referral">
            ...
            <button className="sb-referral-btn" onClick={() => router.push('/profile')}>→</button>
          </div>
          <div className="sb-profile" onClick={() => router.push('/profile')}>
      ...
      {collapsed && (
        <div className="sb-collapsed-bottom">
          ...
          <img src={avatarSrc} className="sb-avatar" alt={firstName} onClick={() => router.push('/profile')} style={{ cursor:'pointer' }} />
